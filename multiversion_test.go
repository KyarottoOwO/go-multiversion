...
func TestProtocols(t *testing.T) {
	...
	if len(protocols) != 0 {
		t.Fatalf("unconfigured historical protocols must not be advertised, got %d", len(protocols))
	}
	if got, want := multiversion.V1_26_44().ID(), int32(2168); got != want {
		t.Fatalf("unexpected protocol ID: got %d, want %d", got, want)
	}
	if got, want := multiversion.V1_26_44().Ver(), "1.26.44"; got != want {
