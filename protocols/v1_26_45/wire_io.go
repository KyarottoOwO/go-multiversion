...
// LegacyShapeData keeps the shared pre-1.26.50 shape schema for older codecs.
func LegacyShapeData(io protocol.IO, value *protocol.ShapeData, reading bool) {
	newWireIO(io, reading).ShapeData(value)
}

// LegacyTextShape is the shared text payload before native line-gap support.
func LegacyTextShape(io protocol.IO, value *protocol.TextShape, reading bool) {
	marshalTextShape(newWireIO(io, reading), value)
}
...
func (io *wireIO) StackRequestItem(value *protocol.StackRequestItem) {
	if io.runtime == nil || io.runtime.blocks == nil || (!io.reading && value.BlockRuntimeID <= 0) {
		io.IO.StackRequestItem(value)
		return
	}
	if io.reading {
		io.IO.StackRequestItem(value)
		if value.BlockRuntimeID == 0 {
			return
		}
...
// line-gap field added to the protocol-2193 native model.
