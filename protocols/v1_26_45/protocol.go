...
// protocol 2169 native release against the protocol-2193 native model.
package v1_26_45
...
// MapBlockRuntimeID maps a protocol-2193 native runtime ID to the frozen
...
func convertWireFromLatest(pk packet.Packet) []packet.Packet {
	...
	case *packet.BossEvent:
		...
		case packet.BossEventRegisterPlayer, packet.BossEventUnregisterPlayer, packet.BossEventRequest:
			// Protocol 2193 removed the target player ID, so these events cannot
			// be represented faithfully for a protocol-2169 client.
			return nil
		}
	case *packet.Disconnect:
		if current.Reason > packet.DisconnectReasonNonceNotValid {
			cloned := *current
...
var packetMarshals = map[uint32]packetMarshal{
	...
	packet.IDPrimitiveShapes:               marshalPrimitiveShapes,
