...
func TestHistoricalOraclePayloads(t *testing.T) {
	...
	heightMap[0][0], heightMap[15][15] = -1, 1
	...
	tests := []struct {
		...
		{name: "dimension_data", pk: &packet.DimensionData{Definitions: []protocol.DimensionDefinition{{Name: "test", MinimumY: -64, HeightRange: 383, Generator: protocol.GeneratorOverworld, DimensionType: 1000}}}, len: 28, hash: "752e49256f086b198beb2c6eaf28ee776ee781b57d65b5a1d7f238ef6066eafc"},
	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, raw packet.Packet) {
				...
				if _, ok := got.PlaybackPositionSeconds.Value(); ok {
					t.Fatal("protocol-2193 playback position leaked into protocol 2169")
				}
			},
		},
		{
		...
		},
		{
			...
			packet: &packet.DimensionData{Definitions: []protocol.DimensionDefinition{{
				Name: "test", MinimumY: -64, HeightRange: 383, Generator: protocol.GeneratorOverworld, DimensionType: 1000, DefaultBiome: "minecraft:plains",
			}}},
			check: func(t *testing.T, raw packet.Packet) {
				got := raw.(*packet.DimensionData).Definitions[0]
				if got.MinimumY != -64 || got.HeightRange != 383 || got.DefaultBiome != "" {
		...
		},
		{
			...
			check: func(t *testing.T, raw packet.Packet) {
				...
				if _, ok := got.StartingRotation.Value(); ok {
					t.Fatal("protocol-2193 starting rotation leaked into protocol 2169")
				}
			},
		},
		{
			...
			packet: &packet.SubChunk{Position: protocol.SubChunkPos{1, 2, 3}, SubChunkEntries: []protocol.SubChunkEntry{{
				...
				HeightMapData: protocol.Option(protocol.HeightMap{}),
			}}},
			check: func(t *testing.T, raw packet.Packet) {
		...
		},
		{
			...
			packet: &packet.PrimitiveShapes{Shapes: []protocol.PrimitiveShape{{NetworkID: 1, ExtraShapeData: &protocol.TextShape{
				Text: "text", LineGapHeight: 2, DepthTest: true,
			}}}},
			check: func(t *testing.T, raw packet.Packet) {
				...
				if shape.LineGapHeight != 0 {
					t.Fatal("protocol-2193 line gap leaked into protocol 2169")
				}
			},
		},
		{
