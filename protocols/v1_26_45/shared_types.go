...
func marshalStackResponseSlot(io *wireIO, slot *protocol.StackResponseSlotInfo) {
	...
	protocol.OptionalFunc(io, &slot.FilteredCustomName, io.String)
...
func marshalDimensionDefinition(io *wireIO, definition *protocol.DimensionDefinition) {
	...
	maximumY, minimumY := definition.MinimumY+definition.HeightRange+1, definition.MinimumY
	...
	if io.reading {
		...
		definition.HeightRange = maximumY - minimumY - 1
...
func marshalMemoryCategory(io *wireIO, value *protocol.MemoryCategoryCounter) {
	...
	io.Uint8(&category)
	if io.reading {
		value.Category = packetconv.MemoryCategory(ID, category, false)
	}
...
func marshalSubChunkEntry(io *wireIO, entry *protocol.SubChunkEntry) {
	...
	protocol.OptionalFunc(io, &entry.HeightMapData, func(data *protocol.HeightMap) {
		packetio.LegacyHeightMap(io.directional(), data)
	})
	io.Uint8(&entry.RenderHeightMapType)
	protocol.OptionalFunc(io, &entry.RenderHeightMapData, func(data *protocol.HeightMap) {
		packetio.LegacyHeightMap(io.directional(), data)
	})
