...
func marshalPrimitiveShapes(io *wireIO, raw packet.Packet) {
	raw.(*packet.PrimitiveShapes).Marshal(io.directional())
}

// LegacyDiagnostics preserves the shared pre-1.26.50 diagnostic entry layouts.
func LegacyDiagnostics(io protocol.IO, memory *[]protocol.MemoryCategoryCounter, entities *[]protocol.EntityDiagnosticTimingInfo, reading bool, targetID int32) {
	legacy := newWireIO(io, reading)
	protocol.FuncIOSlice(legacy.directional(), memory, func(raw protocol.IO, value *protocol.MemoryCategoryCounter) {
		category := value.Category
		if !reading {
			category = packetconv.MemoryCategory(targetID, category, true)
		}
		raw.Uint8(&category)
		if reading {
			value.Category = packetconv.MemoryCategory(targetID, category, false)
		}
		raw.Uint64(&value.Bytes)
	})
	protocol.FuncIOSlice(legacy.directional(), entities, func(raw protocol.IO, value *protocol.EntityDiagnosticTimingInfo) {
		marshalEntityDiagnostic(asWireIO(raw), value)
	})
}
